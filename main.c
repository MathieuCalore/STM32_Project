...
int main(void)
{
  ...
  while (1){
	  if(flags->cardPos==IN){
		  payloadBuffer="hello world";
		  uint8_t syscode=MESSAGE;
		  uint8_t payloadSize=sizeof(payloadBuffer)/sizeof(char);
		  syscodeBufferCoder(syscode,payloadSize);
		  tansmitSyscodeForward(syscodeTransmitBuffer);
		  transmitPayloadForward(payloadBuffer,payloadSize);
	  }else{
		  receiveSyscodeFromPrevious(syscodeReceiveBuffer);
		  treatSyscode(sycodeReceiveBuffer[0],syscodeReceiveBuffer[1],payloadBuffer);
	  }
  }
  /* USER CODE END 3 */
}
